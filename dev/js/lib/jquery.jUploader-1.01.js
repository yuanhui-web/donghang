    ...
    window.jUploader = $.jUploader = function (options) {
        ...
        var complete = function () {
            ...
            try {
                ...
                json = json.replace(/<pre.*>(.*)<\/pre>/g, '$1');
                json = json.replace(/<PRE.*>(.*)<\/PRE>/g, '$1');
                json = json.replace("angular.callback._1000(","");
                json = json.replace("})","}");
                response = eval("(" + json + ")");
            } catch (e) {
                response = {};
            ...
            options.onComplete(options.fileName, response, options.id);
        };
